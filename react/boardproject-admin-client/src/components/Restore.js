...
export default function Restore() {
  ...
  const [deletenBoards, setDeleteBoards] = useState([]); // 삭제 게시글 목록
  ...
  const restoreMember = async (member) => {
    if(window.confirm(member.memberNickname + "님을 탈퇴 복구 시키겠습니까?")) {
      ...
      try {
        ...
        console.log("탈퇴 회원 복구 중 에러 발생 : ", error);
      }
  ...
  const getDeleteBoardList = async () => {
    ...
    try {
      ...
      const resp = await axiosApi("/admin/deleteBoardList");
  ...
  const resotreBoard = async (board) => {

    if(window.confirm(board.boardTitle + " 삭제 복구 시키겠습니까?")) {

      try {
        
        const response = await axiosApi.put("/admin/resotreBoard", {boardNo : board.boardNo});
        ...
        console.log("삭제된 게시글 복구 중 에러 발생 : ", error);
      }
  ...
  useEffect(() => {
    if(withdrawnMembers != null && deletenBoards != null) {
      setLoading(false);
    }

  }, [withdrawnMembers, deletenBoards]);
  ...
  if(loading) {
    ...
    return (
      <div className="menu-box">
        ...
        <RestoreBoard deletenBoards={deletenBoards} resotreBoard={resotreBoard}/>
      </div>
    );
...
const RestoreBoard = ({ deletenBoards, resotreBoard }) => {
  return (
    <section className="section-border">
      <h2>삭제 게시글 복구</h2>
      ...
      <h3>삭제된 게시글 목록</h3>
      {
        deletenBoards.length === 0 ?
        (<p>탈퇴한 회원이 없습니다</p>) :
        (
          deletenBoards.map((board, index) => {
            return (
              <ul className="ul-border" key={index}>
                ...
                <button className="restoreBtn" onClick={() => resotreBoard(board)}>복구</button>
              </ul>
            )
          })
        )
